/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_koutongremote_AndroidRDPActivity */

#ifndef _Included_com_koutongremote_AndroidRDPActivity
#define _Included_com_koutongremote_AndroidRDPActivity
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    getenv
 * Signature: ()Ljava/lang/String;
 */
JNIEXPORT jstring JNICALL Java_com_koutongremote_AndroidRDPActivity_getenv
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    setUsername
 * Signature: (Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_setUsername
  (JNIEnv *, jobject, jstring);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    setPassword
 * Signature: (Ljava/lang/String;)V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_setPassword
  (JNIEnv *, jobject, jstring);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_connect
 * Signature: (Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Z)I
 */
JNIEXPORT jint JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1connect
  (JNIEnv *, jobject, jstring, jint, jstring, jstring, jstring, jstring, jboolean);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdpdr_init
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_com_koutongremote_AndroidRDPActivity_rdpdr_1init
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_main_loop
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1main_1loop
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_disconnect
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1disconnect
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_reset_state
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1reset_1state
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    cache_save_state
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_cache_1save_1state
  (JNIEnv *, jobject);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_send_client_window_status
 * Signature: (I)V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1send_1client_1window_1status
  (JNIEnv *, jobject, jint);

/*
 * Class:     com_koutongremote_AndroidRDPActivity
 * Method:    rdp_send_input
 * Signature: (ISSSS)V
 */
JNIEXPORT void JNICALL Java_com_koutongremote_AndroidRDPActivity_rdp_1send_1input
  (JNIEnv *, jobject, jint, jshort, jshort, jshort, jshort);

#ifdef __cplusplus
}
#endif
#endif
